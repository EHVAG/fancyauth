        ...
        private class BecomeGodfather : IUserContextCallback
        {
            ...
            public async Task Run(IUser actor, IUserShim targetShim)
            {
                ...
                using (var transact = context.Database.BeginTransaction(IsolationLevel.Serializable))
                {
                    ...
                    // if user is already a persistent guest and the actor isn't already a godfather, add new godfather
					if (guest.PersistentGuest != null && !guest.PersistentGuest.Godfathers.Any(g => g.UserId == actor.UserId))
                    {
